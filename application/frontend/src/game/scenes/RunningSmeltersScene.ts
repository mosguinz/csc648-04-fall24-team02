export default class RunningSmeltersScene extends Phaser.Scene {
  ...
  private smeltersPerPage = 3 // Number of smelters to display per page
  ...
  create() {
    ...
    // adding the new pop up window
    const pop_op = this.add.image(650,300,"Screen_Pops_1")
    pop_op.scaleX= 1
    pop_op.scaleY= 1
    pop_op.setInteractive()
    /*
    const background = this.add.rectangle(645, 270, 900, 550, 0x333333)
    background.setInteractive()
    */

    this.add.text(480, 105, "Active Smelters", {
      fontSize: "35px",
      color: "#ffffff",
    })

    const closeButton = this.add
      .text(831, 90, "X", { fontSize: "40px", color: "#ffffff" })
      .setInteractive()
    closeButton.on("pointerdown", () => this.scene.stop("RunningSmeltersScene"))

    // Add left arrow button
    this.leftArrow = this.add
      .text(210, 470, "<", { fontSize: "90px", color: "#ffffff" })
      .setInteractive()
    this.leftArrow.on("pointerdown", () => this.switchPage(-1))

    // Add right arrow button
    this.rightArrow = this.add
      .text(1040, 470, ">", { fontSize: "90px", color: "#ffffff" })
      .setInteractive()
  ...
  displaySmelters() {
    ...
    for (const item of this.smelterContainers) {
      item.destroy()
    }
    ...
    const endIndex = Math.min(
      startIndex + this.smeltersPerPage,
      smelters.length,
    )
  ...
  getRunningSmelters() {
    ...
    if (smelterPlacementScene?.smelterTimers) {
