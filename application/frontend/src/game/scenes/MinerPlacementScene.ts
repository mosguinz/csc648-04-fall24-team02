...
export default class MinerPlacementScene extends Phaser.Scene {
  ...
  create() {
    ...
    const ironNode = this.add
      .image(400, 370, "iron_ore_block")
      .setScale(8)
      .setInteractive()
    const copperNode = this.add
      .image(650, 370, "copper_ore_block")
      .setScale(8)
      .setInteractive()
    const rockNode = this.add
      .image(900, 370, "rock_block")
      .setScale(8)
      .setInteractive()
  ...
  displayMinerInfo(node: string, nodeImage: Phaser.GameObjects.Image) {
    ...
    this.minerTimeTexts[node] = this.add.text(
      nodeImage.x - 40,
      nodeImage.y + 110,
      `Time: ${miningTime}s`,
      {
  ...
  placeMiner(node: string) {
    ...
    if (this.selectedNode === node) {
      ...
      if (inventoryScene.deductFromInventory("iron_ore", 1)) {
        ...
        console.log(
          `Upgraded miner on ${node} node to level ${this.minerLevels[node]} with mining time ${miningTime}ms.`,
        )
