...
import { type Recipe, recipes } from "./Recipes" // Import the recipes data
...
export default class CraftingMenu extends Phaser.Scene {
  ...
  create() {
    ...
    this.leftArrow = this.add
      .text(210, 620, "<", { fontSize: "40px" })
      .setInteractive()
    this.leftArrow.on("pointerdown", () => this.switchPage(-1))

    // Right arrow button
    this.rightArrow = this.add
      .text(1050, 620, ">", { fontSize: "40px" })
      .setInteractive()
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      recipe.ingredients.forEach((ingredient, index) => {
        ...
        const ingredientImage = this.add
          ...
          .setScale(1.5) // Use the item key for the image
  ...
  craftItem(recipe: Recipe) {
    ...
    if (
      recipe.ingredients.every((ingredient) =>
        inventoryScene.deductFromInventory(ingredient.item, ingredient.amount),
      )
    ) {
