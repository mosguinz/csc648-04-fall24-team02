...
export default class TestScene extends Phaser.Scene {
  ...
  create() {
    // Title text
    this.add
      .text(GAME_WIDTH / 2, GAME_HEIGHT / 3, "CLICK AND MORTAR", TITLE_STYLE)
      .setOrigin(0.5, 0.5)
      .setFontSize(128);
    ...
    const startButton = this.add
      ...
      .setInteractive();
    this.add
      .text(startButton.x, startButton.y - 12, "START", TEXT_STYLE)
      .setOrigin(0.5, 0.5)
      .setFontSize(94)
      .setColor("black");
    ...
    const quitButton = this.add
      ...
      .setInteractive();
    this.add
      .text(quitButton.x, quitButton.y - 12, "QUIT", TEXT_STYLE)
      .setOrigin(0.5, 0.5)
      .setFontSize(94)
      .setColor("black");

    // Side arrows for hover effect
    const arrowsContainer = this.add
      .container(startButton.x, startButton.y)
      .setVisible(false);
    const startButtonArrowLeft = this.add
      .image(-400, 0, "button_arrow")
      .setOrigin(0.5, 0.5);
    const startButtonArrowRight = this.add
      .image(400, 0, "button_arrow")
      .setOrigin(0.5, 0.5)
      .setScale(-1, 1);
