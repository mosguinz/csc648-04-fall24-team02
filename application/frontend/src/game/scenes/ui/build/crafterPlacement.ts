  ...
  create() {
    this.add
      .nineslice(
        ...
        "inventory_panel",
        0,
        this.CRAFTER_PLACEMENT_WIDTH,
        this.CRAFTER_PLACEMENT_HEIGHT,
        NSP,
        NSP,
        NSP,
        NSP,
    ...
    const placementContainer = this.add.container(
      GAME_WIDTH / 2 - this.CRAFTER_PLACEMENT_WIDTH / 2,
      0,
    );
    ...
    placementContainer.add(
      this.add
        .text(
          this.CRAFTER_PLACEMENT_WIDTH / 2,
          this.CRAFTER_PLACEMENT_HEIGHT / 7,
          "AUTO-CRAFT",
          TEXT_STYLE,
        )
        .setOrigin(0.5, 0.5)
    ...
    const closeButton = this.add
      .image(this.CRAFTER_PLACEMENT_WIDTH - 35, 35, "close_button")
      .setOrigin(0.5, 0.5)
      .setScale(3)
      .setInteractive();
    ...
    // Crafting slots
    const craftingInput1 = this.add
      .nineslice(
        this.CRAFTER_PLACEMENT_WIDTH / 5,
        this.CRAFTER_PLACEMENT_HEIGHT / 3 + 35,
        "crafting_slot",
        0,
        this.SLOT_SIZE,
        this.SLOT_SIZE,
    ...
    const craftingInput2 = this.add
      .nineslice(
        this.CRAFTER_PLACEMENT_WIDTH / 5 + this.SLOT_SIZE + 5,
        this.CRAFTER_PLACEMENT_HEIGHT / 3 + 35,
        "crafting_slot",
        0,
        this.SLOT_SIZE,
    ...
    const craftingOutput = this.add
      .nineslice(
        this.CRAFTER_PLACEMENT_WIDTH / 1.25,
        this.CRAFTER_PLACEMENT_HEIGHT / 3 + 35,
        "crafting_slot",
        0,
        this.SLOT_SIZE,
    ...
    placementContainer.add(
      this.add
        .image(
          this.CRAFTER_PLACEMENT_WIDTH / 1.7,
          this.CRAFTER_PLACEMENT_HEIGHT / 3 + 35,
          "button_arrow",
        )
        .setOrigin(0.5, 0.5)
    ...
    // Build button
    const craftingButtonText = this.add
      .text(
        this.CRAFTER_PLACEMENT_WIDTH / 2 + 50,
        this.CRAFTER_PLACEMENT_HEIGHT / 2 + 48,
        "BUILD",
        TEXT_STYLE,
      )
      .setOrigin(0.5, 0.5)
      .setFontSize(50);
    const craftingButton = this.add
      .nineslice(
        this.CRAFTER_PLACEMENT_WIDTH / 2 + 50,
        this.CRAFTER_PLACEMENT_HEIGHT / 2 + 55,
        "crafting_slot",
        0,
        140,
        70,
    ...
    craftingButton.on(Phaser.Input.Events.POINTER_OVER, () => {
      ...
      craftingButtonText.setPosition(
        craftingButtonText.x,
        craftingButtonText.y + 5,
      );
    });
    ...
    this.leftArrow = this.add
      .image(
        this.CRAFTER_PLACEMENT_WIDTH / 13,
        this.CRAFTER_PLACEMENT_HEIGHT / 2,
        "left_arrow",
      )
      .setOrigin(0.5, 0.5)
      .setInteractive()
    ...
    this.rightArrow = this.add
      .image(
        this.CRAFTER_PLACEMENT_WIDTH / 1.07,
        this.CRAFTER_PLACEMENT_HEIGHT / 2,
        "right_arrow",
      )
      .setOrigin(0.5, 0.5)
      .setInteractive()
      .setScale(3); // Flip horizontally
    ...
    this.recipeContainer = this.add.container(
      this.CRAFTER_PLACEMENT_WIDTH / 8,
      this.CRAFTER_PLACEMENT_HEIGHT / 2 - 5,
    );
  ...
  private updateRecipeDisplay() {
    ...
    for (let index = 0; index < currentRecipe.ingredients.length; index++) {
      ...
      const ingredientIcon = this.add
        .image(
          this.SLOT_SIZE / 2 + index * (this.SLOT_SIZE + 5),
          0,
          ingredient.item,
        )
        .setOrigin(0.5, 0.5)
      ...
      const ingredientCount = this.add
        .text(
          this.SLOT_SIZE / 2 + index * this.SLOT_SIZE + 20,
          27,
          `${ingredient.amount}`,
          TEXT_STYLE_SMALL,
        )
    ...
    const outputIcon = this.add
      .image(this.SLOT_SIZE * 4.62, 0, currentRecipe.outputItem)
      .setOrigin(0.5, 0.5)
      .setScale(2);
    this.recipeContainer.add(outputIcon);

    const outputCount = this.add
      .text(
        this.SLOT_SIZE * 4.62 + 20,
        25,
