...
export default class BuildMenu extends Phaser.Scene {
    ...
    create() {
        ...
        buildMenuContainer.add(
            this.add.text(this.BUILD_MENU_WIDTH / 2, this.BUILD_MENU_HEIGHT / 15, "BUILD", TEXT_STYLE)
                .setOrigin(.5, .5).setColor("black").setFontSize(64)
        );

        // Close button
        const closeButton = this.add.image(this.BUILD_MENU_WIDTH - 50, 50, 'close_button').setOrigin(.5, .5).setScale(3).setInteractive()
        ...
        // Build miner icon
        const buildMinerIcon = this.add.image(this.BUILD_MENU_WIDTH / 2, this.BUILD_MENU_HEIGHT / 4, 'miner_icon').setOrigin(.5, .5).setScale(6).setInteractive();
        ...
        // Build crafter icon
        const buildCrafterIcon = this.add.image(this.BUILD_MENU_WIDTH / 2, this.BUILD_MENU_HEIGHT / 2, 'crafter_icon').setOrigin(.5, .5).setScale(6).setInteractive();
        ...
        // Build miner text
        buildMenuContainer.add(
            this.add.text(this.BUILD_MENU_WIDTH / 2, this.BUILD_MENU_HEIGHT / 2 + 70, "CRAFTER", TEXT_STYLE_SMALL)
                .setOrigin(.5, .5).setColor("white").setFontSize(32));
    }
}
