...
export default class CraftingMenu extends Phaser.Scene {
  ...
  create() {
    ...
    const craftingContainer = this.add.container(
      GAME_WIDTH / 2.3,
      GAME_HEIGHT / 9,
    );
    ...
    craftingContainer.add(
      this.add
        .nineslice(
          this.CRAFTING_WIDTH / 12,
          this.CRAFTING_HEIGHT / 5,
          "recipe_panel",
          0,
          this.CRAFTING_WIDTH - this.CRAFTING_WIDTH / 6,
          this.CRAFTING_HEIGHT / 2.3,
          NSP,
          NSP,
          NSP,
          NSP,
    ...
    craftingContainer.add(
      this.add
        .text(
          this.CRAFTING_WIDTH / 2,
          this.CRAFTING_HEIGHT / 15,
          "CRAFT",
          TEXT_STYLE,
        )
        .setOrigin(0.5, 0.5)
    ...
    const closeButton = this.add
      .image(this.CRAFTING_WIDTH - 50, 50, "close_button")
      .setOrigin(0.5, 0.5)
      .setScale(3)
      .setInteractive();
    ...
    const craftingInput1 = this.add
      .nineslice(
        this.CRAFTING_WIDTH / 5,
        this.CRAFTING_HEIGHT / 3,
        "crafting_slot",
        0,
        this.SLOT_SIZE,
        this.SLOT_SIZE,
    ...
    const craftingInput2 = this.add
      .nineslice(
        this.CRAFTING_WIDTH / 5 + this.SLOT_SIZE + 5,
        this.CRAFTING_HEIGHT / 3,
        "crafting_slot",
        0,
        this.SLOT_SIZE,
    ...
    this.leftArrow = this.add
      .image(this.CRAFTING_WIDTH / 3.5, 385, "left_arrow")
      .setOrigin(0.5, 0.5)
      .setInteractive()
      .setScale(3);
    ...
    this.rightArrow = this.add
      .image(this.CRAFTING_WIDTH / 1.4, 385, "right_arrow")
      .setOrigin(0.5, 0.5)
      .setInteractive()
  ...
  private updateRecipeDisplay() {
    ...
    for (let index = 0; index < currentRecipe.ingredients.length; index++) {
      ...
      const ingredientCount = this.add
        .text(
          this.SLOT_SIZE / 2 + index * (this.SLOT_SIZE + 5) + 30,
          30,
          `${ingredient.amount}`,
          TEXT_STYLE_SMALL,
        )
    ...
    const outputIcon = this.add
      .image(this.SLOT_SIZE * 4.15, 0, currentRecipe.outputItem)
      .setOrigin(0.5, 0.5)
      .setScale(2.5);
  ...
  private canCraft(recipe: {
    ingredients: { item: string; amount: number }[];
  }): boolean {
    return recipe.ingredients.every((ingredient) => {
      const resource = GameData.resources.find(
        (res) => res.resource_type_id === parseInt(ingredient.item, 10),
      );
  ...
  public craftItem(recipe: {
    ...
    displayFloatingText(
      this,
      recipe.outputAmount,
      parseInt(recipe.outputItem, 10).toString(),
    );
  }
}
