...
export default class GameMenu extends Phaser.Scene {
  ...
  create() {
    ...
    const gameMenuContainer = this.add.container(
      GAME_WIDTH - 10,
      GAME_HEIGHT - 10,
    );

    // Icon hover effect
    const iconHover = this.add
      .image(0, 0, "button_hover")
      .setOrigin(0.5)
      .setVisible(false)
      .setScale(2);
    ...
    this.MENU_CONFIG.buttons.forEach(
      (buttonConfig: ButtonConfig, index: number) => {
        ...
        const buttonText = this.add
          .text(button.x, button.y + 65, buttonConfig.label, TEXT_STYLE_SMALL)
          .setOrigin(0.5)
          .setFontSize(this.MENU_CONFIG.fontSize);
        gameMenuContainer.add(buttonText);

        const buttonIcon = this.add
          .image(button.x, button.y, buttonConfig.icon)
          .setOrigin(0.5)
          .setScale(4);
