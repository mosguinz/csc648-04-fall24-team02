...
from app.models import ResourceBase, ResourceType, UserCreate
...
def test_create_resource_type(db: Session) -> None:
    ...
    new_res = ResourceType(name=name, description=desc, icon_image_url=url)
...
def test_create_resource(db: Session) -> None:
    ...
    res = ResourceBase(resource_type_id=res_id, quantity=count)
...
def test_read_resource(db: Session) -> None:
    ...
    desc = random_lower_string()
    url = random_lower_string()
    new_res_type = ResourceType(name=name, description=desc, icon_image_url=url)
    res_type = crud.create_resource_type(session=db, resource_type=new_res_type)
    res_type_id = res_type.id
    count = random.randint(1, 1000)
    res = ResourceBase(resource_type_id=res_type_id, quantity=count)
    name2 = random_lower_string()
    desc2 = random_lower_string()
    url2 = random_lower_string()
    new_res_type2 = ResourceType(name=name2, description=desc2, icon_image_url=url2)
    res_type2 = crud.create_resource_type(session=db, resource_type=new_res_type2)
    res_type_id2 = res_type2.id
    res2 = ResourceBase(resource_type_id=res_type_id2, quantity=count)
...
def test_update_resource(db: Session) -> None:
    ...
    res = ResourceBase(resource_type_id=res_id, quantity=1)
    new_res = crud.create_resource(session=db, resource_in=res, user_id=user_id)
    resource_in = ResourceBase(resource_type_id=res_id, quantity=count)
    updated_resource = crud.update_resource(
        session=db, db_resource=new_res, resource_in=resource_in
    )
